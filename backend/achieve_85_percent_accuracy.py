...
def create_advanced_features_for_85_percent(df, price_col, date_col):
    """Create advanced features to boost accuracy to 85%+"""
    ...
    for i in range(30, len(df)):
        ...
        # Combine all features (total: 15 + 32 + 8 + 4 + 8 + 8 + 4 + 3 = 82 features)
        feature_vector = (
            lag_features +
            rolling_features +
            momentum_features +
...
def train_high_accuracy_xgboost(X_train, y_train, X_test, y_test, crop, mandi):
    """Train XGBoost with optimized parameters for 85%+ accuracy"""
    ...
    # Save model
    model_path = f"app/data/processed/xgb_85percent_{crop}_{mandi}.joblib"
    os.makedirs(os.path.dirname(model_path), exist_ok=True)
    joblib.dump(final_model, model_path)
    
    return final_model, accuracy

def train_high_accuracy_lstm(X_train, y_train, X_test, y_test, crop, mandi):
    """Train LSTM with advanced architecture for 85%+ accuracy"""
    ...
    callbacks = [
        ...
        ModelCheckpoint(f'lstm_85percent_{crop}_{mandi}_best.h5', 
    ...
    model_path = f"app/data/processed/lstm_85percent_{crop}_{mandi}.h5"
    scaler_path = f"app/data/processed/lstm_85percent_scaler_{crop}_{mandi}.joblib"
...
def achieve_85_percent_accuracy():
    """Main function to achieve 85%+ accuracy"""
    ...
    print("🚀 Starting 85%+ Accuracy Training")
    print("Current: XGBoost ~71%, LSTM ~74%")
    print("Target: Both models 85%+")
    
    for crop in mandis.keys():
        for mandi in mandis[crop]:
            print(f"\n{'='*50}")
            print(f"Training for 85%+ Accuracy: {crop} - {mandi}")
            print(f"{'='*50}")
            ...
            # Check column names and handle date column
            if 'Arrival_Date' in df.columns:
                ...
                print(f"Date column not found in {data_path}")
                continue
            
            # Create advanced features
            X, y, dates = create_advanced_features_for_85_percent(df, price_col, 'date')
            ...
            print(f"Features: {X_train.shape[1]} (advanced features)")
            
            # Train XGBoost
            try:
                xgb_model, xgb_accuracy = train_high_accuracy_xgboost(
            ...
            # Train LSTM
            try:
                lstm_model, lstm_scaler, lstm_accuracy = train_high_accuracy_lstm(
                    X_train, y_train, X_test, y_test, crop, mandi
                )
            except Exception as e:
    ...
    results_df.to_csv('85_percent_accuracy_results.csv', index=False)
    
    # Print summary
    print(f"\n{'='*60}")
    print("85%+ ACCURACY TRAINING SUMMARY")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    results = achieve_85_percent_accuracy()
    
    print(f"\n✅ Training Complete!")
    print(f"Results saved to: 85_percent_accuracy_results.csv")
    print(f"85%+ accurate models saved in: app/data/processed/")
