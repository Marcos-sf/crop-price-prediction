        ...
        if lstm_model is not None:
            ...
            if not os.path.exists(model_path) or not os.path.exists(norm_path):
                ...
                try:
                    ...
                    # LSTM model loading failed, fallback to XGBoost
                    xgb_model = joblib.load(f"app/data/processed/xgb_{crop}_{mandi}.joblib")
                    pred = float(predict_with_xgboost(features, xgb_model)[0])
                    model_type = "xgboost"  # Update model type since we're using XGBoost
    ...
    if xgb_model is None:
        ...
        try:
            ...
            try:
                ...
                # LSTM model not available, use XGBoost only
                lstm_model = None
                lstm_scaler = None
                lstm_target_scaler = None
            ...
            raise HTTPException(status_code=404, detail=f"XGBoost model not found for {crop} in {mandi}: {str(e)}")
